...
WebApp.connectHandlers.use("/admin/export", (req, res, next) => {
  ...
  //   conditionTypes.forEach(ft => puts(_.pick(ft, factorTypeFields)));
  // });

  const factorFields = ["_id", "name", "value", "type", "createdAt"];
  saveFile("factors", factorFields, puts => {
    batch(Conditions)(f => puts(_.pick(f, factorFields)));
  });
  ...
  let playerRoundDataFields = getDataKeys(PlayerRounds);
  saveFile(
    "player-rounds",
    playerRoundFields,
    puts => {
...
//   // console.log(playerInputDataFields);
//   saveFile(
//     "player-inputs",
//     playerInputFields,
//     puts => {
